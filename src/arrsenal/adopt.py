...
def build_plan(found: list[Found], picks: dict[str, str] | None = None) -> Plan:
    ...
    lui, un service detecte plusieurs fois reste ambigu : arrsenal ne devine pas
    ...
    for entry in found:
        if discovery.looks_like_arrsenal(entry):
            skipped.append((entry, "deja gere par arrsenal"))
            continue
        if not entry.usable:
...
def config_from_plan(
    ...
    *,
    ...
    config_root: str,
    host: str = "localhost",
    project_name: str = "arrsenal-adopte",
) -> StackConfig:
    """Materialise le plan en StackConfig.

    Les chemins de donnees restent ceux que l'utilisateur indique : arrsenal ne
    ...
    cfg = StackConfig(
        ...
        data_root=data_root,
        host=host,
        ids_source="stack existante : arrsenal ne gere pas ces conteneurs",
        ids_certain=True,
    )
...
def write_stack(cfg: StackConfig, project_dir: Path) -> Path:
    ...
    path.write_text(
        "# Stack ADOPTEE : arrsenal cable ces services mais ne les gere pas.\n"
