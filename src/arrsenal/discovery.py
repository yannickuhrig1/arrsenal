...
class Found:
    ...
    url_base: str = ""
    #: True quand le conteneur porte le marqueur pose par arrsenal.
...
def scan(*, include_stopped: bool = False) -> list[Found]:
    ...
    for container in containers:
        ...
        entry = Found(
            ...
            managed_by_us=(
                ((container.get("Config") or {}).get("Labels") or {}).get("arrsenal.managed")
                == "true"
            ),
        )
        if entry.host_port is None:
            entry.problems.append(
                f"aucun port de l'hote ne publie {spec.internal_port} : arrsenal ne "
...
def duplicates(found: list[Found]) -> dict[str, list[Found]]:
    ...
    l'animation. arrsenal ne peut pas deviner lequel cabler, il doit demander.
    """
...
def looks_like_arrsenal(entry: Found) -> bool:
    """Ce conteneur vient-il d'arrsenal ?

    On lit le marqueur `arrsenal.managed` pose a la generation, jamais le nom.
    ...
    conteneurs qui n'appartiennent pas a arrsenal.
    """
