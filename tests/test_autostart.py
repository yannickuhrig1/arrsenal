...
from arrsenal import autostart
...
def test_la_commande_reprend_l_interpreteur_courant(tmp_path):
    """Quelqu'un qui a double-clique un executable n'a pas `arrsenal` dans son
    PATH : lui dire « lancez arrsenal serve » ne l'avance a rien."""
    import sys
...
def test_windows_pose_un_script_dans_le_dossier_demarrage(demarrage_windows, tmp_path):
    ...
    cible = demarrage_windows / "arrsenal-console.cmd"
    assert ok
...
def test_le_script_windows_est_lisible_et_explique_comment_le_retirer(
    ...
    contenu = (demarrage_windows / "arrsenal-console.cmd").read_text(encoding="utf-8")

    assert "arrsenal autostart" in contenu
...
def test_windows_retire_le_script(demarrage_windows, tmp_path):
    ...
    assert ok
    assert not (demarrage_windows / "arrsenal-console.cmd").exists()
    assert autostart.status(tmp_path).actif is False
...
def test_l_unite_systemd_a_la_forme_attendue(tmp_path):
    unite = autostart._unite_systemd("/bin/arrsenal serve", tmp_path)

    assert "[Unit]" in unite and "[Service]" in unite and "[Install]" in unite
    assert "ExecStart=/bin/arrsenal serve" in unite
